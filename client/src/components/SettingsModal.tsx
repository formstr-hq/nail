...
function KeyBackup() {
  ...
  if (account.method !== "ncryptsec" || !account.ncryptsec) {
    return (
      <Field
        label="Key backup"
        hint="This account's key isn't stored in the app."
      >
...
export function SettingsModal({ onClose, initialSection }: SettingsModalProps) {
  ...
  const fixedSenderOptions = bridgeAddress
  ...
  const [senderMode, setSenderMode] = useState<string>(() =>
    fixedSenderOptions.includes(initialSenderAddress)
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-5 overflow-y-auto px-4 py-4 md:px-5">
                  ...
                  {account && (
                    ...
                    >
                      ...
                      {!addressesLoading &&
                        ...
                        addresses.length === 0 && (
                          <p className="text-[11.5px] text-subtle">
                            No addresses yet. Your npub address always works —
                            buy a name below.
                          </p>
                        )}
                      {!addressesLoading &&
                    ...
                    returning-owner redirect so it can't bounce back here. */}
                      ...
                      <p className="text-[11px] leading-relaxed text-subtle">
                        Opens the signup page in a new tab. New addresses appear
                        here once paid — reload the list with “Try again”.
                      </p>
                    </Field>
                  )}
              ...
              {section === "help" && (
                <>
                  ...
                  >
                    <a
                      href={`${config.landingUrl}${config.landingUrl.endsWith("/") ? "" : "/"}privacy-policy`}
                      target="_blank"
                      rel="noopener noreferrer"
