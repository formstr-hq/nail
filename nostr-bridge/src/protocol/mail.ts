...
export function buildMailRumor(params: {
  ...
  return buildRumor({
    ...
    extraTags: [
      ...(params.deliverTo ?? []).map((address) => ["deliver", address]),
      ...(params.wrapSecret ? [[WRAP_KEY_TAG, bytesToHex(params.wrapSecret)]] : []),
    ],
  });
}
...
export async function sealAndWrap(
  ...
  opts: { wrapKind?: number; ephemeralSk?: Uint8Array } = {},
): Promise<Event> {
  ...
  let ephemeralSk = opts.ephemeralSk;
