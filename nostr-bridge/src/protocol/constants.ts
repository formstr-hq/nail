...
/**
 * Inner rumor kinds carried over an ephemeral gift wrap. Not mail — the mail
 * decode path skips them and routes on the kind instead (see `unwrapAndVerify`'s
 * `acceptKinds` and the client's receive branch).
 *
 *  - DELIVERY_RECEIPT: bridge → sender, "your mail was handed to Postfix",
 *    content `{ v, messageId, deliveredTo }`. Powers the client's Delivered mark.
