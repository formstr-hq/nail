...
export async function handleMessage(
  ...
  try {
    published = await publishMail({
      ...
      // representation, NOT text-decode it. Mail declares its own charset
      // in Content-Type; decoding as UTF-8 here would corrupt any message
      // that isn't UTF-8 (ISO-8859-1, Shift-JIS, ...) before the parser
      // ever sees the declared charset. See docs/ARCHITECTURE.md §4.
