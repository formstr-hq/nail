...
3. **Router + route merge.** Add react-router; MailApp behind `/app`; delete hand-rolled overlay stack where the router replaces it (keep Android back handler, now fed by router state); move `selfAddresses` into a hook.
