...
FLAP_INSET_X = 6.5        # flap arms stop this far inside the sides, so the
                          # round-capped stroke never pokes past the envelope's
                          # rounded corners (the arms used to reach ENV_L/ENV_R
                          # exactly and spilled over the corner)
...
def render(size: int) -> Image.Image:
    ...
    # Envelope flap — a white V inside the top edge, its arms held clear of the
    # rounded corners (see FLAP_INSET_X).
    fy = ENV_T + FLAP_INSET_Y
    fl, fr = ENV_L + FLAP_INSET_X, ENV_R - FLAP_INSET_X
    draw.line(
        [
            (fl * s, fy * s),
...
def render_splash(w: int, h: int) -> Image.Image:
    """A launch splash: the mark centred on the brand's ink, with room to spare.

    The mark is the full 108-unit icon art (which already carries its own margin),
    scaled to a fraction of the short edge and pasted dead-centre. Because both
    the icon canvas and the splash are INK, the pasted square is seamless — and
    centring the whole canvas guarantees the asterisk, which reaches up and to the
    right of the envelope, is never clipped (the old blue Formstr splash cut it).
    """
    img = Image.new("RGBA", (w, h), INK)
    logo = render(round(min(w, h) * 0.42))
    img.paste(logo, ((w - logo.width) // 2, (h - logo.height) // 2))
    return img


def emit_favicon_svg() -> None:
    """Write the web favicon SVG from the same geometry — the icon as a mark.

    A rounded ink tile (like the launcher's masked square) carrying the red
    envelope, white flap and asterisk, so the web/PWA and in-app glyph match the
    Android icon exactly.
    """
    fy = ENV_T + FLAP_INSET_Y
    cx = (ENV_L + ENV_R) / 2
    fl, fr = ENV_L + FLAP_INSET_X, ENV_R - FLAP_INSET_X

    def ast_lines(width, extra_len, color):
        half = (AST_ARM + extra_len) / 2.0
        out = []
        for ang in AST_ANGLES:
            dx = half * math.cos(math.radians(ang))
            dy = half * math.sin(math.radians(ang))
            out.append(
                f'  <line x1="{AST_CX - dx:.2f}" y1="{AST_CY - dy:.2f}" '
                f'x2="{AST_CX + dx:.2f}" y2="{AST_CY + dy:.2f}" '
                f'stroke="{color}" stroke-width="{width}" stroke-linecap="round"/>'
            )
        return "\n".join(out)

    halo = ast_lines(AST_W + AST_HALO, AST_HALO, INK)
    star = ast_lines(AST_W, 0.0, PAPER)

    # Unlike the launcher icon (which needs a wide safe-zone margin for the mask),
    # the favicon/in-app mark is cropped tight to the art so it doesn't read as a
    # tiny envelope adrift in an ink field at header sizes. This window frames the
    # envelope + asterisk with a small, even margin.
    VB = "24 14 68 68"

    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="{VB}">
  <!-- Mail by Form* mark — GENERATED by scripts/render-app-icon.py. Matches the
       Android launcher icon: red envelope + white flap + Form* asterisk on ink,
       cropped tight for favicon/header use. -->
  <rect x="24" y="14" width="68" height="68" rx="14" fill="{INK}"/>
  <rect x="{ENV_L}" y="{ENV_T}" width="{ENV_R - ENV_L}" height="{ENV_B - ENV_T}" rx="{ENV_RADIUS}" fill="{RED}"/>
  <path d="M{fl},{fy} L{cx},{fy + FLAP_DEPTH} L{fr},{fy}" fill="none" stroke="{PAPER}" stroke-width="{FLAP_W}" stroke-linecap="round" stroke-linejoin="round"/>
{halo}
{star}
</svg>
"""
...
def emit_foreground_vector() -> None:
    ...
    cx = (ENV_L + ENV_R) / 2
    fl, fr = ENV_L + FLAP_INSET_X, ENV_R - FLAP_INSET_X
    flap = f"M{fl},{fy} L{cx},{fy + FLAP_DEPTH} L{fr},{fy}"
...
def main() -> None:
    ...
    # Launch splash — overwrite every existing splash.png in place, at its own
    # size, so the density/orientation set stays exactly as Android expects.
    for splash in sorted(RES.glob("drawable*/splash.png")):
        w, h = Image.open(splash).size
        render_splash(w, h).save(splash)
